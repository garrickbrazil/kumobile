...
$.ajax({url:r,type:"GET",dataType:"html",success:function(t){if(KU.Student.JWEB.isErrorPage(t))return void i("Request failed")
for(var r=$("<div>").html(t),s=$("table.datadisplaytable",r),n=[],a=0;a+1<s.size();a+=2){var o,l,d,c,h,u,m,f,p,v,g,x,b=s.eq(a),w=s.eq(a+1)
courseTitlePattern=/(.*)\s-\s(.*\s.*)\s-\s(\d\d)/
var _=function(t){return t.text().trim().toLowerCase()}
if(courseTitlePattern.test(b.find("caption:first").text().trim())){var y=courseTitlePattern.exec(b.find("caption:first").text().trim())
...
var w=/.*folder_on.*/,_=/.*document_on.*/,y=/.*textbookLogo.*/,q=/contentListItem:(_\d\d+_\d+)/,E=$(".courseId",b).text()
...
w.test(i)?n[n.length]=new e.BB.Folder(t,r,s,a):_.test(i)||y.test(i)?l[l.length]=new e.BB.Item(s,a,E):0==$("> img",this).size()&&(l[l.length]=new e.BB.Item(s,a,E))}})}d=new e.BB.Tree(t,o,n,l),r(d)},error:function(t,e,i){s(i)}})},logout:function(t,i){var r="https://kettering.blackboard.com/webapps/login?action=logout"
